...
export class CacheService {
  ...
  public add<T extends { id: string }>(collection: string, loader: Promise<T[]>) {
  ...
  public async get<T>(collection: string, key: string): Promise<T | null> {
    return this.cacheById[collection]?.then((i) => (i[key] as T) || null);
  }

  public getList<T extends { id: string }>(collection: string): Promise<T[]> {
